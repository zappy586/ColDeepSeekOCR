...
# LlamaAttention / LlamaFlashAttention2 are not imported from transformers because
# the API changed across versions. A self-contained MHA class is defined below
# (after DeepseekV2RotaryEmbedding and apply_rotary_pos_emb are defined).
...
class DeepseekMHAAttention(nn.Module):
    """Standard multi-head attention for DeepSeek-OCR layers where use_mla=False."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.num_kv_groups > 1:
            key_states   = key_states.unsqueeze(2).expand(-1, -1, self.num_kv_groups, -1, -1).reshape(bsz, self.num_heads, -1, self.head_dim)
            value_states = value_states.unsqueeze(2).expand(-1, -1, self.num_kv_groups, -1, -1).reshape(bsz, self.num_heads, -1, self.head_dim)
...
# Aliases kept for any remaining references in the file
LlamaAttention = DeepseekMHAAttention
LlamaFlashAttention2 = DeepseekMHAAttention
